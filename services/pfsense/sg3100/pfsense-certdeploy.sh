...
# Last tested on pfSense Plus version 21.05.1-RELEASE
# This is for the Netgate SG-3100 and has the armv7 GNU binaries included.
...
do
   ...
   then
	   cd /certdeploy/
	   newcrt=$(/certdeploy/base64_armv7 /certdeploy/$domain/fullchain.pem | tr -d '[:space:]')
       newkey=$(/certdeploy/base64_armv7 /certdeploy/$domain/privkey.pem | tr -d '[:space:]')
       cp /conf/config.xml config-sslupdate.xml
       oldcrt=$(/certdeploy/grep_armv7 -A4 -P 'certdeploy' config-sslupdate.xml | awk '/<crt>/ { print $1}' | sed "s|<crt>||g" | sed "s|</crt>||g" | tr -d '[:space:]')
       oldkey=$(/certdeploy/grep_armv7 -A4 -P 'certdeploy' config-sslupdate.xml | awk '/<prv>/ { print $1}' | sed "s|<prv>||g" | sed "s|</prv>||g" | tr -d '[:space:]')
       /certdeploy/gsed_armv7 -i -e "s|$oldcrt|$newcrt|g" config-sslupdate.xml
       /certdeploy/gsed_armv7 -i -e "s|$oldkey|$newkey|g" config-sslupdate.xml
